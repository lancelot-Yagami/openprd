...
test('setup enables Codex hooks while preserving user hook groups', async () => {
  ...
  try {
    ...
    assert.ok(generatedAgents.includes('什么承诺才算真需求'));
    assert.ok(generatedAgents.includes('验证阶段怎样先活下来'));
    ...
    assert.ok(generatedBenchmarkSkill.includes('manual-first delivery'));
    assert.ok(generatedBenchmarkSkill.includes('commitment signals'));
