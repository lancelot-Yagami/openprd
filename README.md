...
- **首轮项目画像**：`clarify` / intake 会先收敛产品形态、目标用户、第一版切片、非目标和保护项，减少一开始就围着局部需求跑偏。
- **评审展示门禁**：新增 `review-presentation`，把 `review.html` 的展示文案校验和稳定快照确认绑在一起。
- **测试策略与执行策略联动**：任务元数据支持 `test-layer`、`test-size`、`evidence-plan`、`execution-mode`、`write-scope`、`local-verify` 和 `integration-owner`，便于主 Agent 编排 worker。
- **历史会话续接更稳**：新增全局 session registry，`继续：<session-id>` 这类工具无关会话 ID 会先做跨 workspace 恢复。
- **基准与经验沉淀更细**：新增 `benchmark observe`、knowledge candidate / draft skill 审查链路，以及 Codex `doctor --tools codex --fix` / `loop --run --agent codex --repair-agent` 预检修复入口。
- **无参考图的界面自检**：`visual-compare` 现在也支持 `--before/--after`，没有设计稿时仍能沉淀修改前后对比证据。
